...
async def start(message: types.Message):
    await message.answer(
        f"Привет, {message.from_user.first_name}.Пожалуйста, введи число, сколько новостей ты хочешь получить")
...
async def count_article(message: types.Message):
    ...
    a = int(user_input)
    await message.answer(f"Хорошо, вы увидите первые {a} популярных новостей за последнее время",
